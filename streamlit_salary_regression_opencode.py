...
st.title("Linear Regression Demo: Du Bao Luong CNTT")
st.caption("App nay chi load va chay model sklearn.linear_model.LinearRegression da train tu data salary.")
...
with st.sidebar:
    ...
    if model_dirs:
        ...
        st.info("Chua tim thay model.joblib nao trong model root folder.")
    ...
    run_folder = st.text_input("Train output subfolder", value="kilocode")
    output_dir = model_root / run_folder.strip() if run_folder.strip() else model_root
    ...
    if st.button("Train lai Linear Regression", type="primary"):
        try:
            with st.spinner(f"Dang train Linear Regression va ghi {display_path(output_dir / MODEL_FILENAME)} ..."):
            ...
            st.success(f"Da ghi model: {display_path(trained_model_path)}")
...
if not model_path.exists():
    st.warning("Chua co model file. Hay train model truoc bang notebook, sidebar hoac CLI.")
    st.code(
...
predict_tab, explain_tab, usage_tab = st.tabs(["Du bao", "Model", "Cach dung"])

with predict_tab:
    st.subheader("Nhap thong tin job de du bao luong")
    ...
    with left:
        ...
        seniority = st.selectbox("Seniority", options=seniority_options, index=seniority_options.index("middle") if "middle" in seniority_options else 0)
    ...
    with right:
        ...
        extra_skill_count = st.slider("So skill khac ngoai top skills", min_value=0, max_value=15, value=0)
    ...
    st.metric(
        "Luong du bao",
        f"{prediction['predicted_salary_million_vnd']:.1f} trieu VND/thang",
    )
    st.caption(f"Predicted log salary: {prediction['predicted_log_salary']:.4f}")

    with st.expander("Xem feature row dua vao model"):
...
with explain_tab:
    ...
    if isinstance(coefficients, pd.DataFrame) and not coefficients.empty:
        st.subheader("Coefficients lon nhat")
        display_columns = [column for column in ["feature", "coefficient", "abs_coefficient"] if column in coefficients.columns]
    ...
    st.subheader("Artifact files in this folder")
    st.dataframe(artifact_status(model_path.parent), use_container_width=True)

with usage_tab:
    st.subheader("Cach train model")
    st.markdown("Cach chinh: mo notebook `notebooks/04_salary_linear_regression_cli_kilocode.ipynb`, chay het cac cell, model se duoc ghi vao folder `kilocode`.")
    st.code("notebooks/04_salary_linear_regression_cli_kilocode.ipynb", language="text")
    st.markdown("Cach phu neu muon train bang terminal:")
    st.code(
        "python -m modeling.salary_regression --input data/analysis/salary_analysis_clean.csv --output-dir data/modeling/salary_regression/kilocode",
        language="bash",
    )

    st.subheader("Cach chay Streamlit")
    st.code("streamlit run streamlit_salary_regression_opencode.py", language="bash")

    st.subheader("Output sau khi train")
    st.markdown(
        """
- `data/modeling/salary_regression/<run_folder>/model.joblib`: model Linear Regression da train.
- `data/modeling/salary_regression/<run_folder>/metrics.csv`: MAE, RMSE, R2.
- `data/modeling/salary_regression/<run_folder>/predictions_test.csv`: actual vs predicted tren test set.
- `data/modeling/salary_regression/<run_folder>/coefficients.csv`: coefficient de giai thich model.
- `data/modeling/salary_regression/<run_folder>/data_audit.csv`: audit data va feature.
"""
    )

    st.info(
        "OneHotEncoder, SimpleImputer va StandardScaler chi la tien xu ly. Model duoc fit van la LinearRegression."
    )
